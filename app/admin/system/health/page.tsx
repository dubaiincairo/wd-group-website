...
export default function SystemHealthAdminPage() {
  ...
  const fetchHealth = async () => {
    try {
      ...
      showToast('Health diagnostics check failed', 'error');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="space-y-8 animate-in fade-in duration-300">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-white/10 pb-6">
        <div>
          ...
          <p className="text-xs sm:text-sm text-zinc-400 mt-1">
            Real-time diagnostics for PostgreSQL latency, Supabase storage buckets, and App Router server state.
          </p>
        </div>

        <button
          onClick={fetchHealth}
          className="inline-flex items-center gap-2 px-5 py-3 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 text-white text-xs sm:text-sm font-bold transition-all"
        >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        <StatCard
          ...
          subtitle={health?.database.provider || 'PostgreSQL'}
          icon={Database}
          iconColor="text-emerald-400"
          badge={health?.database.status === 'healthy' ? 'OPTIMAL' : 'CHECK'}
          badgeColor="bg-emerald-500/20 text-emerald-300 border-emerald-500/40"
        />

        <StatCard
          title="Active Storage Buckets"
          value={health?.storage.buckets.length || 4}
          subtitle="photos, videos, assets, resumes"
          icon={HardDrive}
        ...
        <StatCard
          title="Server Runtime Uptime"
          value={health ? `${Math.floor(health.server.uptimeSeconds / 60)} mins` : '—'}
          subtitle={`Node ${health?.server.nodeVersion || 'v20'}`}
          icon={Server}
          iconColor="text-purple-400"
          badge={health?.server.environment || 'Production'}
      ...
      <div className="bg-[#0F1117]/90 border border-white/10 rounded-3xl p-6 space-y-5 shadow-xl">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="bg-[#141721] rounded-2xl p-4 border border-white/10 space-y-1.5">
            <span className="text-[11px] font-mono text-zinc-400 uppercase">Primary Database Endpoint</span>
            <p className="text-xs font-mono font-bold text-white break-all">
              https://fqkbgfdasfwnryekkgqz.supabase.co
            </p>
            <p className="text-[10px] text-zinc-500">Region: eu-central-1 (Frankfurt, Germany)</p>
          </div>

          <div className="bg-[#141721] rounded-2xl p-4 border border-white/10 space-y-1.5">
            <span className="text-[11px] font-mono text-zinc-400 uppercase">Storage Buckets Verified</span>
        ...
        <div className="text-xs text-zinc-500 font-mono flex items-center justify-between pt-2">
          <span>Diagnostic Run Timestamp: {health?.timestamp || new Date().toISOString()}</span>
          <span className="text-emerald-400">● All health checks passing</span>
        </div>
      </div>
