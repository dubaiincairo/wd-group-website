...
export default function HeroOption5Portals({
  slide,
  currentSlide,
  setCurrentSlide,
  isAr
  ...
  const activeIdx = hoveredIdx !== null ? hoveredIdx : currentSlide;

  return (
    <div className="relative w-full flex-grow flex flex-col justify-center min-h-[82vh] sm:min-h-[88vh] py-6 sm:py-10">
      
      {/* Background Ambient Depth */}
      <div className="absolute inset-0 bg-[#08090C] -z-10" />
      <div className="absolute top-1/3 left-1/2 -translate-x-1/2 w-[800px] h-[400px] bg-[#C9A86A]/5 rounded-full blur-[180px] pointer-events-none -z-10" />
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 border-b border-white/10 pb-6">
          <div className="space-y-2 max-w-2xl">
            ...
            <h1 className="text-2xl sm:text-4xl lg:text-5xl font-black text-white tracking-tight">
              {isAr ? 'مجموعات الأثاث المعماري 2025' : 'Architectural Collections 2025'}
            </h1>
            <p className="text-xs sm:text-sm text-zinc-300 font-normal">
              {isAr 
                ? 'تصنيع سعودي مباشر يجمع بين فخامة الصالونات السكنية، أجنحة الضيافة الفندقية، وهيبة القاعات التنفيذية.'
                : 'Turnkey Saudi manufacturing spanning bespoke luxury residences, presidential hospitality, and corporate boardrooms.'}
            </p>
          </div>
          ...
          <div className="hidden sm:flex items-center gap-2 text-xs text-zinc-400 font-mono">
            ...
            <span className="px-3 py-1.5 rounded-lg bg-[#C9A86A]/10 border border-[#C9A86A]/30 text-[#C9A86A]">
              {isAr ? 'ضمان 10 سنوات' : '10-Yr Guarantee'}
            </span>
          </div>
        </div>

        {/* Triple Expanding Portal Cards */}
        <div className="flex flex-col lg:flex-row gap-4 h-auto lg:h-[530px] w-full">
            ...
            return (
              <motion.div
                ...
                className={`relative rounded-3xl overflow-hidden cursor-pointer border transition-all duration-500 group ${
                  isSelected
                    ? 'lg:flex-[2.4] border-[#C9A86A] shadow-[0_20px_50px_rgba(201,168,106,0.25)] h-[440px] lg:h-full'
              ...
              >
                ...
                <Image
                  ...
                  className={`object-cover object-center transition-transform duration-700 ${
                    isSelected ? 'scale-105 filter brightness-[0.8]' : 'scale-100 filter brightness-[0.55] group-hover:brightness-[0.7]'
                  }`}
                />
                ...
                <div className="absolute top-4 inset-x-4 flex items-center justify-between z-10">
                  ...
                  {isSelected && (
                    <span className="hidden sm:inline-flex items-center gap-1 px-3 py-1 rounded-full bg-[#C9A86A]/20 border border-[#C9A86A]/40 text-[#C9A86A] text-[11px] font-mono font-bold">
                      <span className="w-1.5 h-1.5 rounded-full bg-[#C9A86A] animate-ping" />
                      <span>{isAr ? 'المجموعة النشطة' : 'Active Division'}</span>
                    </span>
                  )}
                </div>
