...
export default function HeroOption6Blueprint({
  slide,
  currentSlide,
  setCurrentSlide,
  isAr
}: HeroOption6Props) {
  const [viewMode, setViewMode] = useState<'3d' | 'elevation' | 'specs'>('3d');
  ...
  return (
    <div className="relative w-full flex-grow flex flex-col justify-center min-h-[84vh] sm:min-h-[88vh] py-6 sm:py-10 overflow-hidden">
      
      {/* Precision CAD Blueprint Grid Canvas Background */}
      <div className="absolute inset-0 bg-[#06080D] -z-10" />
      <div 
      ...
      />
      <div className="absolute top-0 inset-x-0 h-40 bg-gradient-to-b from-[#08090C] to-transparent pointer-events-none -z-10" />
      <div className="absolute bottom-0 inset-x-0 h-40 bg-gradient-to-t from-[#08090C] to-transparent pointer-events-none -z-10" />

      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full space-y-6">
        
        {/* Technical Blueprint Header Metadata Strip */}
        <div className="flex flex-wrap items-center justify-between gap-3 p-3 rounded-2xl bg-[#0F121C]/80 backdrop-blur-xl border border-[#C9A86A]/25 text-xs">
          <div className="flex items-center gap-3 font-mono">
            <span className="text-[#C9A86A] font-bold flex items-center gap-1.5">
              <Compass className="w-3.5 h-3.5" />
              <span>{isAr ? 'مخطط المواصفات الهندسية والتفصيل المعماري' : 'ARCHITECTURAL CAD & MILLWORK DRAFT'}</span>
            </span>
            <span className="text-zinc-500 hidden sm:inline">|</span>
            <span className="text-zinc-400 hidden sm:inline">{`DWG NO: WD-2025-0${currentSlide + 1}`}</span>
            <span className="text-zinc-500 hidden sm:inline">|</span>
          ...
          <div className="flex items-center gap-2">
            <span className="text-[11px] font-mono text-zinc-400">
              {isAr ? 'المصنع: الرياض ونجران' : 'Facility: Riyadh & Najran'}
            </span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8 items-stretch">
          ...
          <div className="lg:col-span-7 relative rounded-3xl overflow-hidden border border-[#C9A86A]/30 bg-[#0B0D14]/90 p-5 sm:p-7 flex flex-col justify-between shadow-2xl">
            ...
            <div className="flex items-center justify-between gap-2 border-b border-white/10 pb-4 mb-4">
              <div className="flex items-center gap-1.5">
                ...
                >
                  ...
                  <span>{isAr ? 'تحليل الهيكل' : 'Joinery DNA'}</span>
                </button>
              </div>
            ...
            <div className="relative h-64 sm:h-80 lg:h-96 w-full rounded-2xl overflow-hidden border border-white/10 bg-black/60 group">
              ...
              {/* Laser Dimension Overlay (Active in 'elevation' or always subtle) */}
              <div className="absolute inset-0 pointer-events-none p-4 flex flex-col justify-between">
                ...
                <div className="w-full flex items-center justify-between border-t-2 border-dashed border-[#C9A86A]/80 pt-1">
                  ...
                  <span className="px-3 py-1 rounded-full bg-[#08090C]/90 text-[#C9A86A] text-[11px] font-mono font-bold border border-[#C9A86A]/40 shadow-lg">
                    {`↔ ${slide.blueprint.lengthCm} cm (طول مخصص)`}
                  </span>
                  <span className="w-2 h-2 rounded-full bg-[#C9A86A]" />
                </div>
            ...
            <div className="space-y-2.5 rounded-2xl bg-black/40 border border-white/10 p-4 text-xs font-mono">
              ...
              <div className="flex items-start gap-2 text-zinc-300">
                ...
                <div>
                  ...
                  <span className="text-[11px] text-zinc-400">{isAr ? 'رفع مقاسات وتعديل 100% مجاناً' : '100% Bespoke shop drawings provided'}</span>
                </div>
              </div>
            </div>
